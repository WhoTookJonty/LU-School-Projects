...
int main(int argc, char** argv) {
	...
	cout << "<Jonathan Wilson> -- CSIS 215 Programming Assignment 1 -- Bag Dictionary" << endl << endl;
	cout << "{Partnered with Jacob Greer}" << endl << endl;

	BDictionary<int, string> myIntStrDict;
	BDictionary<string, int> myStrIntDict;
	...
	cout << "Begin testing of Abag.h functions that were not demonstrated:" << "\n" << "1. inspectTop \n2. += operator" << endl; 

	ABag<KVpair<int, string>> myBag; //Used to test bag functions not previously demonstrated
	
	//Testable KVpair objects 
	KVpair<int, string> kvpair(10, "orange");
	KVpair<int, string> kvpair2(6, "apple");
	KVpair<int, string> kvpair3(8, "pear");

	//Adds kvpair to bag in order to test InspectTop function
	myBag.addItem(kvpair);
	myBag.addItem(kvpair2);

	//Test inspectTop by evaluating if it returns true
	cout << "Testing inspectTop, answer should be 1 (boolean value for true): " << myBag.inspectTop(kvpair) << endl;

	//Test += overloaded operator by evaluating if it increases the vector size when adding an object
	cout << "Testing += operator, the current size of the vector is: " << myBag.size()<< endl;
	myBag += kvpair3;
	cout << "The new size of the vector is: " << myBag.size() << endl;
